...
place 追加（XSRF-TOKEN必要）
```
$ curl -i -b cookie.txt -c cookie.txt -d @place_test.json -H "Content-Type: application/json" -H "x-xsrf-token:2440df59-3b8a-41c6-8c21-0f7b49a943c2" -X POST "http://localhost:8000/app/place"
HTTP/1.1 200 
Vary: Origin
Vary: Access-Control-Request-Method
Vary: Access-Control-Request-Headers
...
Content-Type: application/json
Transfer-Encoding: chunked
Date: Sun, 29 May 2022 08:13:56 GMT
...
place 削除（XSRF-TOKEN必要）
```
$ curl -i -b cookie.txt -c cookie.txt -H "x-xsrf-token:2440df59-3b8a-41c6-8c21-0f7b49a943c2" -X DELETE "http://localhost:8000/app/place/1"
HTTP/1.1 200 
Vary: Origin
Vary: Access-Control-Request-Method
Vary: Access-Control-Request-Headers
...
Content-Length: 7
Date: Sun, 29 May 2022 08:18:23 GMT
```
...
user 追加
```
$ curl -i -b cookie.txt -c cookie.txt -d '@user_test2.json' -H "Content-Type: application/json" -H "x-xsrf-token:2440df59-3b8a-41c6-8c21-0f7b49a943c2" -X POST "http://localhost:8000/app/user"
```

user 更新
```
$ curl -i -b cookie.txt -c cookie.txt -d '@user_test2.json' -H "Content-Type: application/json" -H "x-xsrf-token:2440df59-3b8a-41c6-8c21-0f7b49a943c2" -X PUT "http://localhost:8000/app/user/3"
```

user 削除
```
$ curl -i -b cookie.txt -c cookie.txt -H "Content-Type: application/json" -H "x-xsrf-token:2440df59-3b8a-41c6-8c21-0f7b49a943c2" -X DELETE "http://localhost:8000/app/user/3"
```

ログアウト（POSTなので、XSRF-TOKEN必要）
```
$ curl -i -b cookie.txt -c cookie.txt -H "x-xsrf-token:2440df59-3b8a-41c6-8c21-0f7b49a943c2" -X POST "http://localhost:8000/app/logout"
```
...
権限のないユーザーでログインした場合、一覧は取得できるが
追加、更新、削除はできない
```
$ curl -i -c cookie.txt -X POST "http://localhost:8000/app/login" -d "account=test" -d "password=test"
```
...
> - プレログインAPI未対応  
> 参考サイトにあるプレログインAPIは、最初有効にしましたが、挙動がイマイチで、  
> ログイン時にignoringAntMatchersにしています。  
> 当時はローカル環境で検証しており、実際のSSL対応済みインターネット環境だと  
> 参考サイトどおりに動作するかもしれません。
> - WebSecurityConfigurerAdapter が Deprecated  
> 現行の Spring Boot 2.7.0 で正しく動作するよう、だいぶ修正をしましたが、  
>  Deprecated になっている WebSecurityConfigurerAdapter の回避は未対応です。  
> - この Spring Boot REST構築方法は正しいのか？  
> 実装に際し、上述の参考サイトに従い、バージョンを合わせる対応も大変でしたが、  
> 今、確認すると、spring.pleiades.io の REST API 作成方法が公開されていました。  
> https://spring.pleiades.io/guides/gs/rest-service/  
> Spring Boot は、正しい公式ドキュメントに従った方が良いと思います。